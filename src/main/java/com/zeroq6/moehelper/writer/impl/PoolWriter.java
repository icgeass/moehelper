...
public class PoolWriter implements Writer {
    ...
    private void init() {
        ...
        for (int i = Configuration.getFromPage(); i <= Configuration.getToPage(); i++) {
            ...
            liJsonData.add(ResourcesHolder.getMapIdJson().get(i));
    ...
    private void logPoolChangeToStdErr() {
        MyLogUtils.info("====================Check zip packages changes========================");
        ...
        for (int i = Configuration.getFromPage(); i <= Configuration.getToPage(); i++) {
            ...
            if (PoolLog.POOL_STATUS_MODIFIED.equals(poolLog.getStatus())) {
                ...
                MyLogUtils.info("Pool #" + poolLog.getId() + " was modified, status = " + Integer.toBinaryString(statusNow));
        ...
        MyLogUtils.warn("now the zips should be:");
        MyLogUtils.warn("all: " + (allZipPre + affectedZipNumJpgSum + affectedZipNumPngSum) + ", jpeg: " + (jpegZipsPre + affectedZipNumJpgSum) + ", original: " + (originalZipsPre + affectedZipNumPngSum));
        int jpegZipsNow = 0;
